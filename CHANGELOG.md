...
> ⚠ 如果你在 1.10.2–1.10.7 期间对 PDF 跑过 OCR，这本书的阅读进度百分比、按字数跳转和统计里的「已读字数」都是错的。重新跑一次 OCR 即可修复（会自动跳过已识别的页）。

**安全**
- 电子书 HTML 现在经过消毒才进入阅读器。此前 EPUB / HTMLZ / MOBI / Markdown 的原始 HTML 直接 innerHTML 注入，`<img onerror>` 这类事件属性会以完整应用权限执行 —— 一本从网上下载的书就能读写你的磁盘、拿走 localStorage 里的 API key 和同步 token。已入库的旧书在每次渲染时也会消毒
- 加上了真正的 CSP，`script-src` 不含 `unsafe-inline`
- 文件系统权限从「整个磁盘」收窄到 `Documents/AutoBook`。你自己选的 Obsidian vault 和词典文件夹改为选中时按需授权，用法不变

**数据正确性**
- 修 OCR 完成后进度 / 目录 / 统计三处对不上的根因：OCR 只更新了全书字数，没有同步每页的 section 字数
- 修 AI 剧透截断漏内容：跨越阅读位置的片段之前被整块喂给模型，超长段落更是没有上限；同时修了片段位置偏移的累计误差
- 修「这页已 OCR 过」的判定，之前会把上一页的文本当成本页的，导致该页被永久跳过
- 修删除高亮后关联链接残留悬空 ID；删除文件夹不再让子文件夹变成孤儿
- Obsidian 导出：书名含 `..` 不再写到 vault 外；书名或标签含反斜杠不再产出 Obsidian 解析不了的 frontmatter
- 修大写扩展名（`BOOK.TXT`）导入失败；扩展名不认识的文件现在真的会按文件头识别（1.10.2 起这个功能其实一直不可达）

**稳定性 / 性能**
- OCR 每 25 页存一次盘，中途关掉 App 不再从头再来；语言模型用完即释放（此前常驻内存直到退出）
- MOBI 解析器修两条崩溃路径（畸形文件触发的除零和无限递归）
- 导入 MOBI / AZW3 和调用 Calibre 时不再把整本书变成 JSON 数字数组过桥，大书导入明显变快
- 启动加载的 JS 从 2.9MB 降到 853KB（Markdown 相关的库改为按需加载）
- 设置 → 数据的路径面板不再卡界面，并补上了 localStorage 的大小
- 笔记本同步到 vault 只写有变化的条目
- 词典支持 `idxoffsetbits=64` 的大词典

**其它**
- 同步 token 改走请求头而不是 URL 查询参数（旧版本客户端仍可继续同步）
- AI 默认模型更新为 claude-sonnet-5；OpenAI 兼容接口自动适配新旧两种 token 参数名
- 新增 push / PR 上的 CI（类型检查、构建、自测、Rust 编译）和 `npm test`
