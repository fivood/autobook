...
> ⚠ 高亮右键菜单新增「查词」入口。如果之前的 UI 自定义导致菜单错位，请「重置 UI」后重启。

- 离线词典支持（BYO，自带词典）：选中文字右键 → 高亮菜单新增「查词」→ 浮卡显示所有匹配词典的释义
- 支持 StarDict 格式（.ifo + .idx + .dict / .dict.dz），开放标准，ECDICT / CC-CEDICT / JMdict 等都有现成包
- 支持 *.dict.json 自定义格式，对象 { word: def } 或数组 [{ word, def }]
- 用户在浮卡里点📂选词典文件夹，AutoBook 递归扫描所有 .ifo 和 .dict.json，路径记忆
- 完全离线：.dict.dz 用浏览器原生 DecompressionStream 解压，不依赖联网
- 简单英文词形还原回退：复数/进行时/过去式/比较级查不到时自动尝试还原
