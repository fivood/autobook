...
- **分页模式现在也能用打字机**。以前它只在滚动模式存在。铺到页尾会自动翻页，按下播放从当前页的开头开始铺——不会像早先那样一个字没出来就先翻走一页
- 上一版的「打字机 ↔ 朗读接着同一句继续」在分页模式下同样有效

## 1.48.0

打字机和朗读接得上了。

- **暂停一个再开另一个，会接着同一句继续**。打字机铺到一半停下再点朗读，声音从那一句的开头念起，而不是从视口位置；反过来听到一半停下再点打字机，铺字也从那一句开始，不用从头等。回到的是**句首**——铺字停在半个词上，声音从半句中间起腔是坏的
- 只在滚动模式。分页模式没有打字机，所以没有可交接的对象

## 1.47.0

统计终于会自己记录了。

- **阅读统计默认打开**。以前默认是关的，界面上又没有任何提示——按下朗读听一小时，什么都不会记下来。升级后会自动打开一次；不想要就去 设置 → 统计 → 启用统计 关掉，关掉之后不会再自动打开
- **朗读 / 打字机的时长和字数现在会跨设备同步**。以前只同步总时长和总字数，从另一台设备来的时间在年度页里不属于任何一种阅读方式。手机 PWA 没有朗读，它贡献的时长仍然只进总计、不带归属
- **设置页的字号和按钮统一了**。上游那批面板一直是 16px、后来加的是 12px，中间还夹着八种按钮写法；现在整页一套刻度，Off / On 这类分段控件也不再比周围大一圈

## 1.46.0

听书能挑语气了，内容也终于能缩放。

- **朗读语气可选**：12 种按书的类型写好的语气——小说叙事、悬疑、言情、武侠、科普、商业、儿童、诗歌、学术、剧本、快速过书。只有支持语气指令的服务（小米 MiMo、Gemini TTS、OpenAI gpt-4o-mini-tts）会出现这一栏，选完仍可在请求体里手改
- **暂停时也能调语速了**：以前一按暂停，语速药丸就跟着消失，没法先调好再接着听。现在暂停后药丸留在原地，改完恢复播放就按新速度念
- **朗读中调速立刻生效**：系统语音和 Edge 以前要等当前这句念完才变速，听起来像是没反应
- **Ctrl + 滚轮缩放**：文字书调字号，PDF / 漫画调页面图片。也认 Ctrl `+`、Ctrl `-`、Ctrl `0` 复位，快捷键面板（`?`）里能查到

## 1.45.1

朗读起点跟手了。

- **选中一段文字，按播放就从那里开始读**。以前只要顺手给它上了高亮，起点就会跳回上次停下的位置——高亮的那一步会把选区清掉，朗读于是不知道你指的是哪
- **选中后的右键菜单多了「从这里读」**：不留高亮，直接从这段开读；打字机正在跑的时候，改的是铺字的起点
- **V 快捷键和托盘的播放也认这个设置了**，之前只有右下角那个播放键认
- 选中后的右键菜单靠近窗口右边缘时不再被挤成竖排、右半截跑到屏幕外

## 1.45.0

选完书就地操作，以及一批「原来没有键盘也能用」的补齐。

- **批量操作条移到了书的下方**。选中后底部浮出「导出 / 归档 / 删除」，不用再把鼠标带回窗口右上角
- **书上可以右键**：打开 / 加入分类 / 归档 / 阅读统计 / 删除。右键一本没被选中的书，会先把它变成当前选中——菜单永远作用于打了勾的那些
- **归档可以撤销**，就在提示条上，不用去已归档视图里翻回来
- **选中一批书之后仍可以整批拖进分类**（上一版加框选时被误伤了）
- **`/` 或 Ctrl/Cmd+F 聚焦书库搜索框**
- **排序 / 筛选 / 封面尺寸 / 界面语言这四个菜单现在能用键盘打开**：Tab 聚焦、Enter 展开、Esc 收起。之前它们对键盘完全不可见
- **年度页加了「全部显示」**，关掉几个模块后可以一键复原
- 窄窗口下，书库表头折叠出来的那个按钮不再顶着「返回书库」的图标和提示——它打开的是菜单

## 1.44.1

顶部工具栏的位置分工。

- **「返回书库」固定在最右**，不再随当前页面前后浮动——出口在每一页都是同一个角
- **书库页的工具栏分成了两边**：排序、筛选、封面尺寸、界面语言点开就是一个浮层，留在左边；导入和通往其它页面的入口在右边。「返回当前书籍」也从正中挪到了右边
- **全选按钮挪进了页面**，和「已选 N 本」、加入分类的胶囊在同一行，就在书的上方——它只在书库用得到，不必占着五个页面共用的顶栏

## 1.44.0

书库多选终于像个多选，五个功能页也终于长一个样。

> ⚠ 「全选」以前选的是整个书库，不是屏幕上看到的那些。如果你在搜索或分类下用过「全选 + 删除」，建议顺手确认一下书库还在。这一版已改成只选当前可见的。

- **多选支持拖框选区**。进多选后在书上拖一个框，框到哪选到哪；按住 Ctrl/Shift 拖是在已选中的基础上追加
- **Ctrl / Cmd + 点击**直接进多选，**Shift + 点击**选一段区间；**Esc** 退出多选，**Ctrl/Cmd + A** 全选当前可见
- **多选态现在看得见**：每张卡左上角有勾位，浅色封面上的勾也不会再消失
- **多选时不再显示悬停删除按钮**——它正好压在你要点选的那张卡上
- **全选按钮从表头正中挪到计数旁边**，一进多选就在，不用先选中一本才出现
- **选中的书被筛出当前视图时会写明**「其中 N 本不在当前视图」，因为删除吃的是全部选中项
- **阅读统计 / 高亮笔记 / 翻译工作台 / 设置 / 更新记录共用同一条标题栏**：只有图标和提示，本页的功能在左、去别处的入口在右。这五个页面现在可以直接互相跳转
- **更新记录页不再把 markdown 的星号原样显示**

## 1.43.0

统计这一块的一次补齐：播放算不算阅读、图片书怎么记、以及一整年怎么看。

- **新增「年度」标签页**。以前一整年的数据只能靠导出报告看，现在直接在统计里翻。六个模块——总览、阅读方式、作品结构、时间习惯、笔记与高亮、读得最多——每个都能单独关掉，只留自己在意的
- **朗读和打字机播放时会自动开始计时**，不用先手动点开始。字数按真正读出来 / 显示出来的算
- **统计能分清是朗读、打字机还是手动读的**。时间和字数都分开记，年度页里能看到三者的占比和逐月构成
- **图片书和扫描版按页数统计**。之前它们的页数会被当成字数混进总计，让一本漫画看起来像读了几万字。现在页数和字数分开显示
- **纯图画书之前完全没有阅读量记录**，扫描版记的是页码标签而不是实际页数
- **汇总页的展开按钮点不动**，点了没有反应
...
- **朗读不再偶尔把你甩进未读的段落**。某一句（多半是段落开头）会让页面滚到还没读到的地方，下一句又滚回来。实测一本书里 600 个段落开头有 599 个定位错误，最远偏 65 段；落到章节容器上时会一次跳过几万像素
- **英文书不再用中文音色朗读**。表现是数字被念成中文。书籍语言在保存到书库文件夹时被丢掉了，于是每本书都当成中文处理。已经在库里的书不用重新导入，下次打开就会按正文自动判断语言
- **本机没有对应语言的语音时会说明**。以前会默默换一个语音接着读，看起来像是选错了音色。现在会提示可以改用 Edge TTS（在线语音，无需安装）或补装系统语音包
- **分页模式下朗读的自动翻页更准**。带振假名或隐藏文本的书里，翻页会越走越超前于人声，并且在换章之前不会自行纠正
- **注释小图不再把后面的文字挤到下一行**。有些 EPUB 用一张小图当注释标记，之前它会独占一行

## 1.41.0

一批「看着没事，其实丢东西」的修复。

- **样式表有问题的书不再打不开**。出版社样式表里一个没闭合的注释、少一个花括号，就会让整本书变成空白页，没有任何提示。现在遇到读不懂的样式表就按无样式渲染——书是朴素的，不是打不开
- **编辑器不再点一下背景就丢稿**。书籍正文编辑器和笔记编辑器（编辑已有笔记时），点背景、按 Esc、点取消都会直接丢弃改动。现在会先问一次
- **书库分类计数不再少数几本**。「未分类」的数字按整张分配表算，而删掉的书、另一个书源的书都会在表里留下记录，导致数字偏小、还会冒出点进去是空的分类
- **阅读时长同步不再可能被静默清空**。服务端存的状态一旦读不出来就会重置成空，而客户端不会重新推送已经同步过的日子——两台设备的历史会一起消失。现在改成拒绝并报错，什么都不覆盖
- **自动标签：标签满了不再推荐无效的**。已经有 8 个标签的书会列出可加的标签，勾选应用后却什么都不会发生
- **自动标签：模型回答格式略有出入时不再整批丢弃**，能用的答案会被保留

## 1.40.0

防剧透改成按需功能；AI 请求不再卡死。

- **AI 防剧透改成按书开启，默认关闭**。它只对推理、悬疑这类书有意义，其它书上反而碍事——问一本非虚构「这本书主要论点是什么」，以前会答「目前还没读到，不能剧透」。开关移到 AI 抽屉顶部，只在用 AI 时才看得见，按书记住
- **AI 请求不再无限等待**。模型服务接受连接后卡住（本地模型正忙、代理不通、云端挂起）时，抽屉会一直转圈不报错，批量任务也会停在「运行中」再也起不来。现在会超时并说明原因，本地模型冷加载不受影响
- **防剧透的可见范围算准了**。计算「读到哪」的位置有约 1.45% 的偏差，方向是偏松，30 万字的书大约多放进几页还没读到的内容
- **图片模糊合并成一个选项**。原来是「开/关」加一个藏在后面的「模糊范围」，现在一行三档：不模糊 / 封面外 / 全部。没有插图的书也不再显示这个空分区

## 1.39.0

新增归档；书库、删除和阅读记录的一批修复。

- **新增「归档」**。看完了不想占书库、或者不打算再读，现在可以把书收起来而不是删掉。多选后点工具栏的归档按钮，书从书库消失，侧栏出现「已归档」；进去后同一个按钮变成「放回书库」。归档状态按书名记录，删掉再导入同一本书也不会被莫名藏起来
- **删除书籍不再删掉阅读记录**。读过就是读过——统计和阅读时段一律保留，「删除时保留本地数据」这个开关随之取消
- **「清除僵尸统计」不再误删还在书库里的书**。它判断书是否存在时只看内部数据库，而文件系统书源的书只有被打开过才在里面；实测有三本在架的书会被当成僵尸。现在点击会先列出要删哪些书、多少条记录再确认，名字也改成「清理已删书籍的阅读记录」
- **书库读不出来时不再显示成空书库**。移动硬盘没插、文件夹改名、授权失效，以前都表现为「书一本都没了」且不会重试；现在会说明原因和文件夹位置
- **换书库文件夹时，选到当前书库的子目录不再崩溃**。之前会把整个书库反复复制进自己，直到进程栈溢出
- **朗读不再永久卡住**。Edge TTS 的代理链路半开时不报错也不出声，那一句会无限等待，重试机制根本轮不到执行；现在 20 秒超时后自动重试
- **代理地址以分号结尾时不再探测失败**。Windows 系统代理常写成这种格式，此前会导致 Edge TTS 报 TLS 握手失败
- **Calibre 转换超时后清理临时文件**，不再在系统临时目录里留下整本书

## 1.38.4

漫画消字的失败提示，加朗读、词典、OCR 的三个修复。

- **漫画消字：LaMa / IOPaint 连不上时会说明**。以前静默降级——背景均匀的气泡照常用本地填充消掉，但复杂背景的气泡原文还留着，页面上看不出哪里不对。现在警告区写清楚有几个气泡没处理、连不上的原因
- **朗读的当前句标记不再和用户高亮撞色**。改成波浪下划线 + 极淡底色，压在自己的笔记高亮上也分得清
- **词典弹窗不再每查一次词就泄漏一个订阅**。查得多了，每次词典库变动都会在一堆已关闭的弹窗上重跑一遍
- **整本 OCR 全部失败时不再往书里写任何东西**。上一版只堵了一半：失败的哨兵段落仍会写进书、推到外部存储，并让「检测到扫描版 PDF」的横幅从此消失

## 1.38.3

OCR 和漫画处理的「失败了却说成功」。

- **整本 OCR 一页都没识别出来时，不再显示「OCR 完成」**。以前 300 页全失败和全成功的提示一模一样，而且这本书会被记进「已 OCR」名单，「检测到扫描版 PDF」的横幅再也不出现——等于没有回头路。现在会说明有多少页失败、第一个失败原因，并且全失败时不进名单，下次还能再试
- **整本 OCR 完成时不再回滚运行期间改的设置**。这个过程要几分钟，横幅一直在屏幕上：期间在横幅里改识别语言、或右键单页重 OCR，结果都会在 OCR 结束时被覆盖回去
- **漫画消字失败时不再显示「准备就绪」**。整页消字失败之前没有任何提示，现在在警告区说明。端点留空仍然是「关闭」，不会误报

## 1.38.2

接着上一版：把「出错了但看不出原因」剩下的几处堵上。

- **错误提示不再越出错说得越少**。之前只要这次运行里出过一次错（启动时一条无关的字体告警就够），之后所有错误框都会把具体原因换成一句套话——导入失败只说「书籍导入期间发生错误」，数据库失败只说「发生错误」。现在原因照常显示，「下载报告 / 提交报告」按钮也照常提供
- **批量导入会列出每一个失败的文件**。原来一次导入多本、几本失败，只报得出最后那本，其余要反复重试才发现。现在逐个列出（超过 10 个折叠成「…还有 N 个文件失败」）
- **笔记目录同步失败终于有地方看**。这个同步只会自动跑，而它的结果提示全部挂在一条从未启用的手动分支上，所以一直什么都不报。现在在设置 → 数据的同一栏显示原因，例如「计划新增 2 条笔记，实际只导入 0 条」。上一版声称修好了这条，实际没生效，抱歉

## 1.38.1

三处「出错了但你看不到」。

- **笔记目录同步不再谎报成功**。同步完成提示里的「新增 N」用的是*计划*条数而不是实际导入条数，所以笔记一条都没进库时，提示仍然显示成功。现在报实际条数，导入失败会说明计划几条、实际几条、什么原因
- **后台自动同步阅读时长出错会告诉你**。手动点上传/下载本来就会报错，但真正在跑的自动同步（启动时、每 5 分钟、统计变化后）失败时什么都不显示，唯一线索是「上次同步」的时间戳悄悄不动。现在在设置 → 数据的同一处显示原因，不弹框
- **词典加载失败会说明是哪个文件、为什么**。原来只说「N 个词典没能加载」，具体原因（缺 .idx / .dict 文件、JSON 哪里格式错）查不到

## 1.38.0

朗读、书签、统计的一批修复。统计的默认行为有变化，见第四条。

- **朗读时不再逐字显示正文**。原来开朗读会把全书藏起来再跟着语音逐字放出来，可它跟不准——两个引擎都不知道语音真实读到哪，只能靠估算。现在正文一直完整显示，只把**当前朗读的那句**高亮出来，页面也能正常手动滚动了（滚动后会暂停自动跟随 8 秒，不跟你抢）
- **朗读失败会告诉你原因**。以前引擎报错（代理不通、Kokoro 没下载、自定义接口 key 填错等）只是按钮自己弹回去，什么都不说
- **在书的开头设的书签现在跳得回去**。滚动和分页模式都受影响：书签存的位置本身是对的，是读取时把「位置 0」当成了「没有书签」
- **统计默认开启空闲检测（5 分钟）**。原来只要阅读器开着就一直算时长，人离开也照涨；现在超过 5 分钟没有滚动/指针/选择就停止计时，并把这段空闲从已计入的时长里扣回。**这条会对老用户生效一次**（此前空闲检测默认是关的），想改回去或调整分钟数在「设置 → 统计 → 空闲时间」。另外那里的「最大 12 小时」以前因为单位写错基本不生效，现在对了
- **换书后不再残留上一本书的高亮**。在两本书之间来回切时，新书上可能短暂画出上一本的高亮区间，或者侧栏一直列着另一本书的高亮

## 1.37.0

阅读时能改的设置从一项变成一整套。

- **阅读页的设置抽屉现在分「朗读 / 排版 / 阅读」三页**。字号、行高、段落间距、排版方向、阅读区边距、书签、页脚显示这些原来都要跳去设置页再跳回来，现在在书上直接改，改完立刻看到效果
- 设置页本身没有减少任何东西，两边用的是同一套面板，习惯从设置页进的照旧

## 1.36.1

修 1.36.0 的窗口标题，顺带给托盘的重置项加了确认。

- **窗口标题不再停在旧版本号**。之前标题里的版本是手写的，忘了改就一直停在 v1.33.0；现在从应用版本自动取，不会再对不上
- **托盘右键的「重置 UI」现在会先确认**。它就在「退出」上面一格，原来单击即擦除全部界面设置并重启，误点没有任何挽回余地。写入失败时也会明确报错，不再是「点了没反应」
- **设置文件损坏不再导致应用打不开**。localStorage 里任何一项设置写坏（掉电、磁盘满写了一半），原来整个界面直接白屏，只能靠托盘重置把所有设置一起清掉；现在只有坏掉的那一项回落成默认值，其余照旧

## 1.36.0

菜单按「书库 / 读书」分家，朗读音色按语言记。

- **阅读页新增朗读设置抽屉**。点顶栏齿轮不再整页跳去设置页再跳回来，引擎、音色、试听、语速、起点、快捷键都在书上直接改，改完下一句就生效
- **音色按语言记，中日英各一档**。打开一本书自动套用这本书语言对应的音色——日语书读日语音色，中文书读中文音色，不用每换一本书就去改一次。四个引擎各记各的；老用户原来的音色设置照旧，动了新开关才开始分档
- **阅读页顶栏图标重排**。右侧从九个图标收成五个：完成、全屏、更多、设置、返回书库，统计 / 跳转 / 翻译收进「更多」菜单；图片库挪到左边，和目录、高亮、书签这些看书用的放一起
- 书库顶栏菜单补上「翻译工作台」入口
...
- **统计默认看本月，不再是只看今天**。补一条过去日期的阅读记录，原来页面上什么都不变，看起来像是没保存成功。已经自己选过时段的用户不受影响
- **会话页现在显示含手动补录的时段总计**。原来这一页只统计自动记录的阅读时段，补录完切过去只会看到「此时段没有会话记录」。现在分成「本时段记录（含手动补录）」和「会话（仅自动记录）」两块，各自标注
- **统计页顶栏改成真正的标签条**。原来标签和「手动添加」「导出报告」这些动作是同一排一模一样的图标，只有选中的那个有底色——所以点「手动添加」会以为要切页，其实是开弹窗。现在标签带文字、选中时反色，动作留在右侧
- **手动添加对话框去掉预填内容**。字数栏原本预填了一个 0，得先删掉才能输入；作者、出版社那些格子里的示例文字也一并去掉了
- **修复对话框展开后顶部跑出屏幕**。展开「书籍信息」后弹窗比窗口还高，标题和关闭按钮都够不着。现在标题和按钮固定，只有中间的表单滚动
- 高亮页不再显示「手动添加」——那一页不展示阅读记录，按钮点了没有任何反馈

## 1.34.1

修 1.34.0 的高亮配色。

- **修复换了配色方案后，只有正文变了**。右键菜单的色块、侧栏和笔记页的色点、统计的色条都还是原来那四种固定颜色——反色模式下正文已经是单色，这些地方仍显示黄蓝绿粉；自定义模式选了什么颜色，除了正文以外也完全看不出来
- **色块上印了槽号**。反色模式四个槽共用一种颜色，靠填充和下划线区分，但 12-16px 的小圆点撑不住这种区分。印上编号后识别和配色无关，取色器也终于有了标签
- 设置里「高亮样式」那块的字号和取色器尺寸对齐了页面其余部分
...
- **新增「高亮样式」**（设置 → 外观），三选一：
  - **彩色**——和以前一模一样，默认不变
  - **反色（无障碍）**——四个槽全部用主题的正文色，不靠颜色区分，改用**填充和下划线**：实心反色 / 实线 / 点线 / 淡底细线。起因是原来的绿和粉在绿色盲（约 6% 的男性）眼里几乎是同一个颜色，实测色差 1.6，等于分不出来；亮度差异不受任何色盲影响，所以干脆不用色相
  - **自定义**——四个槽各自取色
- **修复深色主题下高亮的下划线其实没画出来**。1.33.0 说给深色主题加了实色下划线来补偿变淡的底色，实际上那条线被样式覆盖成了透明，所以深色主题一直只有一层很淡的底，颜色并不好分。现在真的画出来了
- 已有的高亮不受影响，换样式只是换外观，不动内容

## 1.33.0

笔记使用手感。

- **深色主题下的高亮不再压垮正文**。原来所有主题共用同一档半透明色块，浅色主题是深字压亮底没问题，深色主题却是浅字压亮块——实测正文对比度从 7.2 掉到 1.6，划了重点反而比不划更难读。现在深色主题改用很淡的底色加一条实色下划线，颜色照样分得清，字也读得清了
- **正文里能看出哪条高亮有备注**：带备注的高亮词尾多一个小圆点，**鼠标悬停就浮出备注卡片**。以前要读备注得先打开侧栏或点进编辑框，也就是说你得先知道有备注才可能去看
- **笔记导出到 vault 改为增量**：内容没变的文件不再重写，不会在 Obsidian 的同步和备份里造成一堆假变更
- **修复导出文件改名后的残留**：文件名带正文摘要，改过高亮范围后会换名，旧文件原本会一直留在 vault 里。现在会清理掉。手动放进该目录的文件、以及本地已删除笔记对应的文件都不会被动

## 1.32.0

**笔记目录同步**（设置 → 数据 → 笔记目录同步）。选一个存放 md / txt 笔记的目录——比如 Obsidian vault 里的某个子目录——每次打开书库时自动同步进来。

- **单向，以文件为准**。在 Obsidian（或任何编辑器）里改完，回到 AutoBook 就是新的；阅读器只读，不会往你的文件里写任何东西
- **子目录变成分类**，层级由你的目录结构决定
- **改内容不丢进度**：只重新渲染正文，阅读位置、高亮、分类都保留
- **移动和改名能认出来**：文件换了目录或改了名字，只要内容没变，就只更新位置，不会变成「删掉再导入」
- **删除会同步**，但有阅读记录或高亮的书不会自动删——那种情况会列出来让你自己确认，因为「改名的同时又改了内容」在文件系统看来和删除一模一样
- 同步来的书在阅读器里不能编辑（会被下次同步覆盖），请在原文件里改
- 自动跳过 `.obsidian`、`.trash`、`AutoBook`（笔记导出目录）和非笔记文件

选目录必须用「选择目录」按钮，不能手打路径——这是 1.29.0 收窄文件权限后的要求。

## 1.31.0

- **从文件夹导入时，按目录结构自动建分类**。子目录的相对路径就是分类名（`读书/技术`），层级深浅由你的目录结构决定，不做截断。适合把 Obsidian vault 之类已经整理好的目录整个拖进来
- **分类侧栏分成两组**：上面是你在阅读器里手建的分类（带新建按钮），下面是从本地目录引入的。目录一深就会产生几十个分类，混在一起会把自己建的那几个埋掉。没做过文件夹导入的话，下面这组不显示
- 多选导入和从 zip 解包出来的书没有目录信息，照旧不分类

## 1.30.0

书库外观。

- **书卡左上角常驻显示格式标签**（EPUB / PDF / MD / MOBI / 漫画…）。这个标签其实一直存在，但只在鼠标悬停时才浮现、而且在右上角——等于要先指着一本书才能知道它是什么格式。「已读 / 未读」徽章让位到右上角
- **没有封面的书，默认封面重做**：按格式着色并放大格式名，加了书页轮廓图案，缩略图大小下不看清字也能分辨。此前格式名画在卡片底部、正好被标题栏挡住，所以这类书看起来就是一块纯色
- **漫画（CBZ / CBR / CB7 / CBT）和 KFX 现在能正确识别格式**，之前这几种连悬停都不显示
- **格式配色跟随主题**：深色主题用深色封面配亮字，浅色主题用淡色封面配深字；自定义主题不用做任何设置也会自动适配。同时修正了标签文字的对比度——旧配色下白字在所有格式上都偏淡
- **主题列表不再半透明**：未选中的主题原来压到 50% 透明度显示，看到的是任何主题都没有的颜色。现在按真实颜色显示，选中态用粗边框区分

## 1.29.0

安全加固与导入性能。

> ⚠ **如果你设过自定义书库目录、Obsidian vault 或词典文件夹，升级后首次启动会提示重新授权**——把原来那个文件夹再选一次就好，之后不再提示。在重新选择之前，这些目录的同步会保持停止，本地书库不受影响。

- **文件读写权限从「整个磁盘」收窄**到 `文档/AutoBook` 加上你自己选过的文件夹。上面的重新授权提示就是这个改动带来的
- **书内容清洗堵掉一处绕过**：用实体编码伪装的 `javascript:` 链接（形如 `java&#9;script:`）此前能穿过 1.28.5 的清洗。改为白名单判定，并且提前到解析阶段执行，恶意书没有抢跑的机会
- **更新说明和 Markdown 实时预览也纳入清洗**，此前这两处未经处理
- **同步 token 不再出现在 URL 里**，改走请求头（旧版本客户端仍可正常同步，无需同时升级）
- **打开 Markdown 书更快**：代码高亮从约 190 种语法收窄到 21 种常见语言，相关资源体积降低约 65%；未收录的语言按纯文本显示
- **导入 MOBI / AZW3 明显更快**：书籍内容不再以 JSON 数字数组跨进程传递
- **支持 `idxoffsetbits=64` 的大词典**，这类词典此前查出来是乱码
...
- **剩余硬编码中文接入 i18n**：高亮侧栏/备注、词典、OCR 状态、图片缩放、导出、主题编辑器、管理器取消操作与 toast、旧域名提示等 50+ 处硬编码中文全部接入三语言词条，切换语言后不再残留中文
- **书卡详情与拖拽提示 i18n**：书卡的相对时间（x 分钟前/万字）、详情弹层标签、拖入分类提示补齐三语言
- **危险操作/错误色统一走 `--danger-color`**：删除、移除等危险操作和错误提示原来硬编码 `text-red-500/600`，换主题后危险色不跟随。新增 `.text-danger`/`.hover-danger`/`.border-danger` 工具类并全量替换，深浅主题自适应
- **中性灰边框/分隔线改 currentColor**：统计页、设置页、侧栏的 `border-gray-500/*`、`bg-gray-500/*`、`bg-gray-400` 等硬编码灰改为 `currentColor` 半透明（`border-current/*`/`bg-current/*`），波纹效果、分类侧栏选中态同步走主题变量，深色主题下不再「灰底灰字」或 hover 不可见
- **图标按钮补 `aria-label`**：阅读器 header 与统计 header 的纯图标按钮原来只有 `title`，屏幕阅读器读不出名称。补 `aria-label`（`title` 保留作 hover 提示）

## 1.28.3

- **分页模式内存占用下降**：分页阅读器原来把整本书解析成 DOM 树常驻内存（`sections: Element[]`），与正文 HTML 字符串、图片 blobs 形成同一内容的三份内存，几十万字的大书 DOM 树是字符串的数倍。改为初始化解析时只提取每章 `id`、`innerHTML` 字符串和字数，DOM 树即弃；翻页只做字符串切片，锚点/章节跳转按需解析单章
- **顺带消除翻页卡顿源**：每章字数统计原来挂在 `SectionCharacterStatsCalculator` 构造里，而它每次翻页（`onHtmlLoad`）都重建并遍历**全书 DOM** 跑 Unicode 正则——每次翻页 O(全书)。字数改为初始化时算好缓存，之后每次翻页只做 O(章节数) 累加
...
- **修复漫画导入残留旧状态**：先导入文本书再导入 CBZ 时，界面不再残留上一本书的文件名/语言/段落数（导入漫画时完整清空旧的文档/任务状态）
- **翻译工作台图标更换**：双向箭头 → 卷轴（header 入口与漫画翻译 banner 一致，含义更清晰）

## 1.28.0

语言路由 + 模型分离：中/英/日/韩四语 OCR 与「初译用翻译专用模型、精校用通用 LLM」的完整流程。核心是新增语言路由层——根据源语言、页面类型（文字/扫描/漫画）、质量档位，自动决定用哪套 OCR、哪个初译模型、是否精校，并可手动覆盖。

- **语言路由层**：按源语言自动分流。韩文强制专用 OCR 模型，英/日/韩/欧自动进入「翻译到中文」，中文书只 OCR 不自我翻译；质量档位（快速/均衡/高质量/手动）决定初译与精校配置
- **韩文 OCR（v5-korean）**：接入官方 Korean PP-OCRv5 识别模型（det 复用 v5）。韩文模型未安装时**阻止任务并提示安装**，绝不静默用其他语言模型识别韩文（避免生成看似正常但错误的文字层）；漫画 OCR 缓存键含模型档位，韩文不与中/日缓存串用
- **翻译模型能力评分**：默认初译模型从「参数最大」改为「翻译专用优先」——本地装了 Hy-MT2 会自动选它，否则多语通用，最后才按参数；精校保持通用聊天模型
- **Qwen-MT 云端初译**：新增翻译专用 provider（source_lang/target_lang/术语 terms/领域 domains），设置 → AI 新增 Qwen-MT 配置区；云端初译用 Qwen-MT，精校仍用 Anthropic/OpenAI 通用模型
- **初译/精校本地模型独立**：精校的本地模型可单独选择（默认通用模型），不再与初译（Hy-MT2 优先）共用
- **回归集**：新增 `npm test`（28 用例），覆盖四语 OCR 场景矩阵、模型评分、Qwen-MT 请求结构、路由校验

## 1.27.2

这一版统一了菜单与弹层的视觉规范，并为笔记、Markdown 和 TXT 书籍加入共用的文字编辑体验。

- **菜单与弹层样式统一**：统一常用菜单项、工具栏、弹出菜单和设置控件的字号、间距、圆角、边框与选中态，改善小字和深浅主题下边框不一致的问题
- **笔记 Markdown 编辑器升级**：笔记支持编辑、分栏预览、纯预览和标题目录跳转；保留标签、颜色与草稿恢复，并修复保存后草稿定时器可能重新写回旧内容的问题
- **Markdown / TXT 正文编辑**：阅读器菜单新增“编辑正文”；Markdown 支持实时预览与目录跳转，TXT 提供轻量纯文本编辑，支持 `Ctrl/Cmd + Enter` 保存
- **保留原始文本并兼容旧书**：新导入的 MD/TXT 会保存原始源文本；旧版本导入的书可从现有阅读内容重建，保存前会明确提示检查内容和书签/划线偏移风险
- **正文保存与外部存储同步**：保存后重新生成正文 HTML、字数和目录，并同步更新外部存储书库；可恢复到首次编辑前的原始版本
- **清理开发 warning**：补齐交互层语义、类型导入和 SCSS modern compiler 配置，`svelte-check` 达到 0 errors / 0 warnings

## 1.27.1

大书打开/导入的加载进度补丁（修复 1.27.0 的 storage-pull 进度未生效 + 导入体验细节）。

- **修复打开外部存储大书时进度卡 10%**：1.27.0 的进度回调挂在了浏览器存储读取上（IDB 快，不触发），真正的外部磁盘解压没传回调。改为把解压进度接入阅读器进度条（从存储拉取 10% → 50% 持续推进），不再像冻住
- **导入进度节流 + 确保 50%**：大书 per-entry 回调太快导致中间进度被跳过、只看到 10%，节流到 ~150ms 让进度条可见推进，并强制结束显示 50%
- **导入取消立即生效**：扫描 PDF 逐页渲染时，点取消约 10 秒才停止。改为 Promise.race 监听 AbortSignal + `doc.destroy()` 释放资源，取消即时生效
- **导入初始 ETA 显示「准备中…」**：不再显示无意义的 `??:??:??`，进度条旁新增百分比（如 23%）
- **新增 `npm run tauri:dev:cdp`**：一键启动带 CDP 调试端口（9223）的 tauri dev，供自动化测试连接

## 1.27.0

大书导入与阅读体验优化 + 一个 overlay 修复。

- **大书导入加速**：扫描 PDF 逐页渲染原来串行 await，长 PDF 要等很久。改为并发渲染（`pLimit(3)`），同一本 32MB 扫描 PDF 导入提速约 25%；PDF 文档解析支持 AbortSignal，导入中断立即生效不再干等
- **导入进度推进**：PDF 导入时逐页上报进度，进度条实时走、ETA 倒数（之前大 PDF 卡在 `~ ??:??:??` 像卡死）
- **大文件存储提示**：导入超过 100MB 的文件且当前用浏览器存储时，提示建议启用外部文件存储（不自动改存储源）
- **阅读器渲染加速**：图片库排序原本对每张图做一次全文档 indexOf（几百页大书是 O(n²)），改为在替换占位符时一次扫描记录位置（O(n)）
- **修复漫画 overlay 不显示**：重新 OCR 会创建空任务（无译文但时间戳最新），原逻辑按最新任务选导致已翻译的旧任务被遮蔽。改为优先选「有译文」的任务，已翻译漫画的覆盖层恢复正常
...
- **漫画气泡抹字（M2）**：工作台 OCR 后自动把气泡原文擦掉（采样背景色填充，纯色气泡干净抹除，实测 86% 覆盖），阅读器里译文叠在干净底图上
- **手工修订**：阅读器右上角「编辑」进入修订模式，拖拽框选漏检的 SFX/艺术字即时抹除（可撤销）；点两个气泡可交换阅读顺序（欧美左→右/日漫右→左）
- **LaMa 抹字（M3）**：设置 → OCR 填 IOPaint 端点后，渐变/纹理气泡和 SFX 交给 LaMa 重绘（保留背景，不再纯色破坏），实测复杂气泡从跳过提升到全处理；sidecar 不可达自动回退纯色
- **模型管理**：设置 → AI 顶部新增四模型卡片（PaddleOCR/Kokoro/Ollama/LaMa），显示就绪状态、下载进度、存储位置、卸载；Ollama/LaMa 是外部进程给清理指引
- **PaddleOCR 下载进度**：OCR 模型下载时实时显示字节进度，下载完成持久化标记
- **修复漫画 overlay 页号错位**：跳过封面时译文曾整体叠错一页（OCR 切片索引与全书页号没对上）
- **修复模型管理状态不刷新**：从 Map 改响应式 store，各模型状态实时更新

## 1.24.0

这版打通了漫画翻译的最小闭环：CBZ / CBR 漫画从书库进翻译工作台 → OCR 识别文字 → 归组成气泡 → 本机模型翻译 → 回阅读器看译文覆盖层。欧美漫画（大写词、并排气泡）和扫描 PDF 都覆盖。

- **漫画翻译管线（M1 闭环）**：CBZ/CBR 进工作台自动 OCR（PaddleOCR，支持 19 种语言），OCR 结果按页缓存到文件系统、中断续跑不重跑；气泡归组 → AI 校正确认 → 分批翻译 → 阅读器里译文按气泡坐标叠在页面上（`comic-overlay`），懒加载/缩放自动重绘
- **按「能否选中文字」分流**：进工作台先判断书有没有文字层——漫画和无文字层扫描 PDF 走 OCR 管线，有文字层的书（EPUB/TXT/MD/文字层 PDF）直接翻译，不再按文件扩展名武断分流
- **OCR 设置独立成页**：设置顶部新增「OCR」标签页，PDF 与漫画翻译各一套默认语言；书库里的书可以记住自己的 OCR 语言（per-book 覆盖，日语漫画设一次以后不用再改）
- **漫画阅读器入口**：漫画打开时顶部出现「打开翻译工作台」banner，点击直达；已有译文的书自动隐藏 banner，直接看覆盖层
- **术语表候选优化**：欧美漫画常见的大写词（强调全大写、句子开头大写、`The X` 短语）不再误列为专名候选，真专名（人名/地名）仍保留；不校准的候选直接用默认翻译，不阻塞
- **翻译分批可调 + 自动恢复**：设置里可调「单批段数 / 单批源字符上限」，本地小模型可调小更稳；中断后重新打开工作台自动恢复继续
- **气泡归组改进**：并排气泡（欧美漫画常见）用水平重叠比例区分，不再被并成一个；overlay 字号按气泡高度比例，不再溢出对白框
- **无文字层的书隐藏 AI 助手按钮**：AI 防剧透助手靠文字层建索引，漫画/未 OCR PDF 上按钮不再出现（OCR 完成后自动恢复）
- 修复书库顶部按钮在窗口缩到 1280 以下时错位（header 不再被 896px 容器收窄，中宽屏菜单折叠成 Popover）
- 修复翻译菜单图标和切换应用语言图标重复（翻译改用双向箭头，语言保留地球图标）
- 修复漫画模式下「导出」按钮可点但报错（改回阅读器查看译文）

## 1.23.2

- 设置 → AI 新增翻译工作台默认配置（目标语言、初译/精校来源、本机模型、分块字数），从书进入工作台时自动填入；工作台内仍可逐项覆盖，改过的字段带重置按钮

## 1.23.1

- 翻译工作台的初译和精校都可以自由选模型来源（本机 Ollama 或云端），不再限定初译只能 Ollama、精校只能 OpenAI 兼容接口

## 1.23.0

这版把 AI 接进了阅读流程，并新增翻译工作台。AI 相关功能都要自己配模型（本地 Ollama 或云端 API），没配的话相关入口不会出现，其余功能照旧。

- **新增翻译工作台**：从书库某本书或直接拖入文件建任务，先校准专名译法，再用本机模型批量初译，可选云端模型精校，最后导出 EPUB / HTML / TXT 或 Markdown 分块。断点自动保存，中断和关掉应用都能接着跑
- **划词查义**：在词典释义下面多一节，只解释这个词在当前这句话里的意思，而不是罗列所有义项。本地模型优先，句子不出本机
- **扫描版 PDF 的文字层纠错**：用模型修 OCR 认错的字（己/已/巳、未/末）、漏掉的标点、中文之间多出来的空格。页面图像不动，改善的是选中、搜索、朗读和 AI 检索的准确度
- **书籍自动打标**：从书籍文件自带的主题词整理出标签，没有模型也能用；配了模型的话，文件里没写主题词的书也能补上。标签要勾选确认才写入，不会覆盖你手填的
- **书库统计新增作者维度**：导入的 EPUB 会读取文件里的作者、出版社、出版年，和手动录入的信息分开存，重新导入不会覆盖你改过的
- **音色可用性检查**：换了设备或系统更新后，之前选的朗读音色如果没了会明确提示并可一键换，不再静默不出声
- AI 配置统一到设置 → AI 一处，本地和云端可以同时配，各功能按需要自动选
- 修复只有一个章节的 EPUB 导入失败
- 修复切换页面时界面会先闪一下别的语言
- 设置里补上界面语言切换（在「外观」里），菜单选中样式和其他地方统一
- 修复深色主题下部分边框显示为固定浅灰、不跟随主题

## 1.22.2

- Edge TTS 连不上时，报错会直接告诉你怎么办：已经配好代理却仍失败，多半是代理软件把 `bing.com` 归到了「Microsoft」分组而该分组默认走直连。设置里也补了同样的说明和修法
- 整本 PDF 的 OCR 跑完后会释放识别引擎占用的内存，不再驻留到关闭应用为止。之后再用 OCR 约需 1 秒重新就绪
- 问过 AI 的书不再无限占用内存，只保留最近两本的检索索引
- 清理了大量历史遗留的无用代码，安装包与内存占用略有下降

## 1.22.1

- 修复 Edge TTS 在需要代理的网络下完全无法使用（报 `tls handshake eof`）。现在会自动跟随系统代理，也可在 Edge TTS 设置里手填代理地址，留空为自动、填 `direct` 强制直连；连接失败会自动重试，并给出可操作的提示
- Kokoro 模型下载状态现在看得见：下载完成有明确提示，卡住超过 30 秒会告警并可一键重试，重开应用也会自动确认模型是否还在
- Kokoro 新增「删除本地模型」按钮，不再需要走「清除全部本地数据」才能腾出磁盘
- 自定义 HTTP TTS 选中后可以直接试听了（之前点预览没有任何反应）
- 去掉 TTS 引擎与自定义 TTS 预设列表里的 emoji 装饰
- 修复朗读快捷键在启动时偶发「已注册」冲突
- 修正 Kokoro 说明文案：320 MB 指 v1.1-zh 的下载体积，82M 是模型参数量而非磁盘占用

## 1.22.0

- 新增 Edge TTS 引擎：直接白嫖微软 Edge 浏览器「大声朗读」用的免费 WSS 端点，不用账号、不用付费、不用绑卡。300+ 神经网络音色，晓晓/云扬/云希/小艺等中文音色和 Azure Neural TTS 同源，日语（七海/圭太）、粤语（曉曼）、台灣國語、韩语都齐。设置里选「Edge TTS」引擎即可用，默认晓晓。需能连 speech.platform.bing.com
...
- 打字机按下播放不再卡顿。之前 30 万字的书要冻结 1.3 秒才出第一个字，现在 55 毫秒
- 朗读句与句之间的空白基本消失（实测 469 毫秒 → 15 毫秒），越慢的引擎（Kokoro、网络 TTS）收益越明显
- 分句更准：小数点、英文缩写、中文引号不再被切断，「他说：「你来了。」」不会把右引号甩到下一句开头
- 超长无标点段落不再被整段跳过（以前会直接丢内容），改为在逗号处切开逐段读
- 修朗读高亮和揭字位置随阅读进度逐渐偏移的问题
- 修 Windows 系统语音语速被应用两次：设 1.5 倍速实际按 2.25 倍速播放，设 2 倍变 4 倍
...
- **对着 Calibre 验证**：3 个抽样 joint KF8 文件（章节数分别为 187 / 39 / 11）跑 `ebook-convert` 得到 epub，再拿 Phase 3 输出对比每节 xhtml 的**去 tag 后 text 字符数**——三个文件都是 100%/101%/101.5%，字节级 rounding-error 匹配。HTML 总字节数差 -5% 到 +12% 是因为 Calibre serializer 会加/删自己的 `calibre_pb` 锚点、`id`、`aid` 等属性，与内容正确性无关
