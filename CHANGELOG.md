...
- 微信读书导出的书里，「注」标记改为跟随阅读主题和字号，不再是固定的黑圆点
- 悬停「注」时，注释出现在标记旁边，配色跟随主题，不再是窗口底部的黑框
- 修：分页模式下，每章最后一页悬停「注」看不到注释

## 1.51.0

菜单整理：常用的留在手边，偶尔才用的收起来。

**阅读中**

- 打开外文书不再先弹「没有可用的语音」，按朗读时才提示
- 选中文字右键多了「复制」和「问 AI」，问 AI 会把这段文字带进 AI 助手
- 顶栏精简：完成本书、阅读点、编辑正文收进 ⋮ 菜单；速度显示改为「字/秒」；书里的齿轮改叫「阅读设置」
- 漫画和还没 OCR 的扫描本不再显示用不上的高亮、目录按钮

**书库**

- 顶栏从 14 个图标减到 8 个：三个导入合成一个「导入」，封面尺寸并入「显示」；界面语言在「设置 → 外观」里改，更新历史和检查更新移到「设置 → 数据」
- 右键菜单和多选操作条的动作一致了（补上导出、清除统计），分类收进「加入分类」，菜单不会再超出窗口
- 修：桌面端书库里「阅读统计」「清除统计」一直不出现
- 排序菜单可以用键盘选择了
...
- **修：分页模式打字机播完一章就停，点播放也没反应**。现在会自动翻到下一章从开头接着打；开了「播完本章停止」的照旧停在新章开头
- **修：翻到下一章时落在第三页**。一章最后一页撑不满一屏时，进入下一章会整体偏移两页，语音自动翻页也受影响
- **修：打字机跨章后偶尔一直转圈**

## 1.50.1

漫画和扫描本打开时不再自己弹提示，顺手修了一个张冠李戴的翻译 bug。

- **漫画 / 扫描本打开时默认不再弹 OCR 和翻译提示**。需要的话在「设置 → OCR」里打开；已经手动开过的人不受影响
- **翻译工作台移到阅读器头部**，不用再到「更多」菜单里找
- **修：没翻译过的漫画会顶用别的书的翻译**。角标写着「N 段译文」但那是另一本书的数字，点「编辑」编的也是那本书的任务，气泡还可能落到这本书的页面上。只要书库里有过一本翻译过的漫画，其余漫画都会中招

## 1.50.0

大漫画导入快了一大截，进度条终于会动了。

- **超大漫画导入快了约 3.6 倍**：那本 991 MB / 185 页的书从 138 秒降到 38 秒。页面重编码换了更快的编码器并挪到后台线程，存盘的写法也换了
- **导入进度条现在真的会走**：以前解析和写盘两个阶段压根不报进度，条子停在原地、右边一直是 `~ ??:??:??`。现在全程在动，剩余时间也是算得出来的
- **进度条配色跟随主题**：之前是浏览器自带的灰板，和界面不搭

> 已经在书库里的书不会被动，上面的变化只影响新导入的。

## 1.49.1

超大漫画不再黑屏。

- **修超大漫画（近 1 GB、每页 5–8 MB 的高清扫描）导入或打开时窗口变黑**。不是渲染失败，是渲染进程被内存挤掉了——窗口壳还在、里面空了，所以连一句报错都没有
- **超过 1.5 MB 的漫画页在导入时重编码到最长边 2400px**，和 Calibre 处理漫画的做法一样。实测那本 185 页的书从 997 MB 降到 115 MB，屏幕上看不出差别；小于这个尺寸的页面逐字节不动，普通漫画完全不受影响
- **漫画和扫描版 PDF 的导入快了一截**：打包时不再对已经压缩过的图片做一遍无用的二次压缩，同一本书这一步从 70 秒降到 10 秒

> 已经在书库里的漫画不会被改动，重编码只发生在新导入的时候。
