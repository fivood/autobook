...
- **AI 检索的 chunk 偏移必须是真实偏移**。`chunkBookText` 的 `startChar` 是防剧透闸门的依据（`retrieveSpoilerSafe` 靠它筛掉读者还没读到的段落），不是记账字段。别再用「`split(/\n+/)` 再按 `para.length + 1` 累加游标」推算——`htmlToPlaintext` 在块级元素前后各发一个 `\n`，每个空行间隔就丢一个字符，实测 400 段偏 1.45%，而且方向是**偏小**：还没读到的内容会显得更靠前，直接穿过闸门
...
- 加场景的前提是**它对应一个真实发生过的回归**。想不出对应的 bug，就说明那个
  场景大概率不值得写
- 唯一的例外：**功能的失败形态是「书凭空消失」这一类用户无法自查的**。归档
  （1.39.0）是第一个按这条加的——过滤反了、归档标记比书活得久、侧栏那行不出现，
  三种 bug 在用户眼里长得一模一样，都是「我的书没了」，而这个症状这个应用已经
  产生过两次（外存后端读不出来显示空书库、CSS 解析失败整页空白）。**破例要在
  场景注释里写明理由**，不要默默加
- `js` 标签模板会**吃掉正则转义**：它是 tagged template，`\s` `\d` 到手里已经变成
  `s` `d`，正则于是永远匹配不上——不报错，只是安静地一直返回 null。表达式里有正则
  转义时用普通模板字符串，值自己 `JSON.stringify`。这条坑掉过一次
- 套件是**绕过应用直接写 IDB** 的，所以收尾后应用内存里的 store 可能还是旧值
  （比如侧栏还显示「已归档 1」）。那不是 bug，刷新即消
