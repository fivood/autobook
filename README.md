...
Windows 桌面电子书阅读器，把读书变成可检索、可回顾的个人知识库。原型 fork 自 [ttu-ebook-reader](https://github.com/ttu-ttu/ebook-reader)，与上游已实质性分叉。Tauri 2 + SvelteKit + Rust，NSIS 安装包，带签名的自动更新通道。
...
- **PDF**：通过 PDF.js 提取文本 + 整页图，按页切 section
...
首次安装 SmartScreen 会提示"未知发布者"，点"更多信息 → 仍要运行"。
